...
def get_patch_complexity(pt):
    pt = np.max(pt, axis=-1)
    gre = graycomatrix(pt.astype(np.uint8), distances=[1], angles=[0], levels=256, symmetric=True, normed=True)
    level = graycoprops(gre, 'contrast')
    return level[0, 0]
...
class HDRPU21:
    ...
    Reference:
    Rafał K. Mantiuk and Maryam Azimi, "PU21: A novel perceptually
    uniform encoding for adapting existing quality metrics for HDR",
    ...
    def encode_tensor(self, x):
        device = x.device
        dtype = x.dtype
        L_min = torch.tensor(self.L_min, dtype=dtype, device=device)
        L_max = torch.tensor(self.L_max, dtype=dtype, device=device)
        p = torch.tensor(self.par, dtype=dtype, device=device)

        x = torch.clamp(x, min=L_min, max=L_max)
        pow_x = torch.pow(x, p[3])
        V = p[6] * (torch.pow((p[0] + p[1] * pow_x) /
                              (1 + p[2] * pow_x), p[4]) - p[5])
        return V

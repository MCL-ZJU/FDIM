...
class video_reader:
    ...
    def __init__(self, vidfile, frames=-1, resize_fn=None, resize_height=-1, resize_width=-1, verbose=False):
        ...
        if 'nb_read_frames' in video_stream:
            ...
            # from timestamp to seconds '00:00:10.000000000'
            duration_str = video_stream['tags']['DURATION']
            from datetime import datetime
            t = datetime.strptime(duration_str[:11], '%H:%M:%S.%f').time()
            # 再换算成秒（含小数）
            duration = t.hour * 3600 + t.minute * 60 + t.second + t.microsecond / 1_000_000
    ...
    def _setup_ffmpeg(self, vidfile, resize_fn, resize_height, resize_width, verbose):
        ...
        stream = ffmpeg.output(stream, 'pipe:', format='rawvideo', pix_fmt=out_pix_fmt).global_args( '-loglevel', log_level, '-nostdin' )
    ...
    def _npuint16_to_torchfp32(self, np_x_uint16, device):
        ...
        np_x_int16 = torch.tensor(np_x_uint16.astype(np.int16), dtype=torch.int16)  # 导致有负数产生
        torch_x_int32 = np_x_int16.to(device).to(torch.int32)
        torch_x_uint16 = torch_x_int32 & max_value  # 负数进行补码运算
...
class video_reader_yuv_pytorch(video_reader):
    ...
    def unpack(self, x, device):
        ...
        RGB = Yuv_float @ ycbcr2rgb.transpose(1, 0)  # H, W, C
...
class video_source_video_file(video_source_dm):
    ...
    def close(self):
        if hasattr(self, "test_vidr") and self.test_vidr is not None:
            self.test_vidr.close()
            self.test_vidr = None
        if hasattr(self, "reference_vidr") and self.reference_vidr is not None:
            self.reference_vidr.close()
...
class video_source_file(video_source):
    ...
    def close(self):
        if hasattr(self, "vs") and self.vs is not None and hasattr(self.vs, "close"):
            self.vs.close()
        self.vs = None
